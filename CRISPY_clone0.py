...
newCSV = 'CRISPRi_library0_overview.csv'
feature_dict = {}
                ...
                if len(NT) <= 23:
                    ...
                    for seq_number, seq in enumerate([NT, T]):
                        ...
                        for nuc_number in range(target_start, target_end, increment): #iterate through half of the nucleotides
                            ...
                            if PAM in theor_PAM_list:
                                sgRNA = seq[nuc_number + 3:nuc_number + 23].reverse_complement()  # sgRNA is upstream of PAM
                                if len(sgRNA) < 20:
                                    continue
                                print("Found PAM at position "+str(abs(nuc_number-target_start+2))+"/"+str(len(seq)) +" targeting strand "+target_strand)
                                offtarget = 0
                                for nucleotide in range(7,len(sgRNA)): #iterate through all nucleotides of sgRNA
                                    for item in record_list: #iterate through all gb-files
                                        fwd = re.findall(str(sgRNA)[7:nucleotide]+r'\w'+str(sgRNA)[(nucleotide+1):], str(item.seq))
                                        rev = re.findall(str(sgRNA)[7:nucleotide]+r'\w'+str(sgRNA)[(nucleotide+1):], str(item.seq.reverse_complement()))
                                        #find all the 0 and 1 bp mutated offtargets
                                        offtarget += len(fwd) + len(rev)
                                if offtarget >= 14: #if more than 1 offtarget -> check next sgRNA
                                    continue
                                offtarget = 0
                                for nucleotide in range(7, len(sgRNA)-1): #iterate through all nucleotides of sgRNA -1
                                    for nucleotide2 in range(nucleotide+1,len(sgRNA)): #iterate through all nucleotides of sgRNA -2
                                        for item in record_list: #iterate through all gb-files
                                            fwd = re.findall(str(sgRNA)[7:nucleotide]+r'\w'+str(sgRNA)[nucleotide+1:nucleotide2]+r'\w'+str(sgRNA)[(nucleotide2+1):],str(item.seq))
                                            rev = re.findall(str(sgRNA)[7:nucleotide]+r'\w'+str(sgRNA)[nucleotide+1:nucleotide2]+r'\w'+str(sgRNA)[(nucleotide2+1):],str(item.seq.reverse_complement()))
                                            # find all the 0,1 and 2 bp mutated offtargets
                                            offtarget += len(fwd)+len(rev)
                                if offtarget >= 89: #if more than 10 offtargets -> check next sgRNA
                                    continue
                                else:
                                    PAM_counter += 1
                                    ...
                                    twobp_mismatch_list.append(offtarget-78)
                                    if offtarget >= 79:
                                        twobp_counter += 1
                    ...
                    with open(dir+'/'+seq_type_list[record_number]+'/'+locus+'_sgRNAs.csv', 'w', encoding='utf-8') as new: #save sgRNA-list under feature name
                        ...
                        for PAM_num, PAM_PAM in enumerate(PAM_list):
                            w.writerow([sgRNA_list[PAM_num], PAM_list[PAM_num], strand_list[PAM_num], twobp_mismatch_list[PAM_num],sgRNA_position_list[PAM_num]])
                        new.close()
